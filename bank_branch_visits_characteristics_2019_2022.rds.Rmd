...
acs <- acs[,c("censustract","mean_family_income","college_or_higher_frac","refinance_rate","dividend_frac","median_age")]
...
rm(irs)
rm(new_acs)
rm(new_acs_housing_units)
rm(new_hmda_summary)
rm(acs_housing_units)
rm(acs_block_data)
gc()
```

```{r}
visits_file = paste0(data_path,
                     c('Dewey/cbgs_place_month_visits_count20190101_20200301.csv.gz',
                       'Dewey/cbgs_place_month_visits_count20210101_20221201.csv.gz'))
visits <- rbindlist(lapply(visits_file, fread))
gc()
# visits <- fread(visits_file[1],nrows = 10)
...
rm(visits)
setkey(visits_sum,censustract)
setkey(acs,censustract)
gc()
```
...
place_visitor_wa <- merge(place_visitor_wa,sod[,c("NEW_PLACEKEY","UNINUMBR")],by="NEW_PLACEKEY")
place_visitor_wa[,DATE_RANGE_START:=as.Date(DATE_RANGE_START)]

```

```{r}
saveRDS(place_visitor_wa,paste0(data_path,"Dewey/bank_branch_visits_characteristics_2019_2022.rds"))
```


```{r}
place_visitor_wa[,yr:=year(DATE_RANGE_START)]
```


```{r}
for(year in unique(place_visitor_wa$yr)) {
  temp <- place_visitor_wa[yr==year]
  temp[,yr:=NULL]
  saveRDS(temp,paste0(data_path,paste0("Dewey/bank_branch_visits_characteristics_",year,".rds")))
